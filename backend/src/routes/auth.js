...
router.get('/users', authMiddleware, async (req, res) => {
  const db = getDb();
  const users = await db.prepare('SELECT id, email, name, role, position, area, created_at FROM users ORDER BY created_at ASC').all();
  res.json(users);
});
...
router.post('/users', authMiddleware, async (req, res) => {
  ...
  const id = uuidv4();
  await db.prepare('INSERT INTO users (id, email, password_hash, name, role, position, area) VALUES (?, ?, ?, ?, ?, ?, ?)')
...
router.put('/users/:id', authMiddleware, async (req, res) => {
  ...
  await db.prepare('UPDATE users SET name = ?, role = ?, position = ?, area = ? WHERE id = ?')
    .run(name ?? user.name, role ?? user.role, position !== undefined ? position : user.position, area !== undefined ? area : user.area, req.params.id);
  const updated = await db.prepare('SELECT id, email, name, role, position, area, created_at FROM users WHERE id = ?').get(req.params.id);
  res.json(updated);
});
...
router.post('/users/bulk', authMiddleware, async (req, res) => {
  ...
  for (const u of users) {
    ...
    const existing = await db.prepare('SELECT id FROM users WHERE email = ?').get(u.email.toLowerCase().trim());
    if (existing) {
