...
router.post('/', authMiddleware, async (req, res) => {
  ...
  const id = uuidv4();
  await db.prepare('INSERT INTO surveys (id, title, description, anonymous, measurement_type, created_by) VALUES (?, ?, ?, ?, ?, ?)')
    .run(id, title, description || '', anonymous ? 1 : 0, mtype, req.user.id);

  if (questions && questions.length > 0) {
    for (const [i, q] of questions.entries()) {
      await db.prepare('INSERT INTO questions (id, survey_id, type, text, options, required, order_index) VALUES (?, ?, ?, ?, ?, ?, ?)')
        .run(uuidv4(), id, q.type, q.text, q.options ? JSON.stringify(q.options) : null, q.required ? 1 : 0, i);
    }
  }

  if (mtype === 'continuous') {
    const slug = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') + '-' + Math.random().toString(36).slice(2, 7);
    await db.prepare('INSERT INTO waves (id, survey_id, name, slug, status) VALUES (?, ?, ?, ?, ?)')
      .run(uuidv4(), id, 'Medición continua', slug, 'open');
  }

  const survey = await db.prepare('SELECT * FROM surveys WHERE id = ?').get(id);
  const qs = await db.prepare('SELECT * FROM questions WHERE survey_id = ? ORDER BY order_index').all(id);
...
router.put('/:id', authMiddleware, async (req, res) => {
  ...
  await db.prepare('UPDATE surveys SET title = ?, description = ?, anonymous = ?, measurement_type = ? WHERE id = ?')
    .run(title || survey.title, description ?? survey.description, anonymous ? 1 : 0, mtype, req.params.id);

  if (questions) {
    await db.prepare('DELETE FROM questions WHERE survey_id = ?').run(req.params.id);
    for (const [i, q] of questions.entries()) {
      await db.prepare('INSERT INTO questions (id, survey_id, type, text, options, required, order_index) VALUES (?, ?, ?, ?, ?, ?, ?)')
        .run(q.id || uuidv4(), req.params.id, q.type, q.text, q.options ? JSON.stringify(q.options) : null, q.required ? 1 : 0, i);
    }
  }

  const updated = await db.prepare('SELECT * FROM surveys WHERE id = ?').get(req.params.id);
  const qs = await db.prepare('SELECT * FROM questions WHERE survey_id = ? ORDER BY order_index').all(req.params.id);
  qs.forEach(q => { if (q.options) q.options = JSON.parse(q.options); });
